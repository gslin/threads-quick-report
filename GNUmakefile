...
$(FIREFOX_ZIP): src/manifest.json src/content.js $(ICONS)
	rm -rf build/firefox
	mkdir -p build/firefox/icons
	cp src/manifest.json build/firefox/
	cp src/content.js build/firefox/
	cp $(ICONS) build/firefox/icons/
	cd build/firefox && zip -r ../../$@ manifest.json content.js icons/

$(CHROME_ZIP): src/manifest.json src/content.js $(ICONS)
	rm -rf build/chrome
	...
	cd build/chrome && zip -r ../../$@ manifest.json content.js icons/
